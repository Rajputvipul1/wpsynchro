    ...
    public function prepareSQLQueries()
    {
        ...
        foreach ($this->job->from_dbmasterdata as $table) {
            ...
            if($table_name == 'wp_posts'){
                $sql_queries_in_group[]= "DELETE FROM `$table_name` WHERE post_type = 'attachment'";

                $sql_queries_in_group[] = "REPLACE INTO `$table_name` SELECT * FROM `$table_temp_name` WHERE post_type = 'attachment'";


                $postChangesIds=$this->getPostChangesIds();
                // error_log('get post changes id s'. print_r(  $postChangesIds,true));
                if(!empty($postChangesIds['added_post_ids'])){
                    $addedRows=$postChangesIds['added_post_ids'];
                    foreach($addedRows as $rows  ){
                        
                        $sql_queries_in_group[] = "REPLACE INTO `$table_name` SELECT * FROM `$table_temp_name` WHERE ID = $rows";
                        $sql_queries_in_group[] = "REPLACE INTO `$table_name` SELECT * FROM `$table_temp_name` WHERE post_parent = $rows AND post_type = 'revision'";
                   
                  
                    }
                }
if(!empty($postChangesIds['deleted_post_ids'])){

    $deletedRows=$postChangesIds['deleted_post_ids'];
    
    foreach($deletedRows as $rows  ){
        
        $sql_queries_in_group[]= "DELETE FROM `$table_name` WHERE ID = $rows";
    }
}
        ...
        // error_log('sql queries: ' . print_r( $sql_queries, true));
    ...
    public function getPostChangesIds(){

 // Handle preserving data
 $sql_queries = [];
 $sql_queries['added_post_ids']=[];
 $sql_queries['deleted_post_ids']=[];
 $sql_queries_last = [];

 // Handle data to keep
 // $sql_queries[] = $this->handleDataToKeep();

 // Get post IDs from "post changes" table
 global $wpdb;
 $post_changes_table = $wpdb->prefix . 'post_changes'; // Adjust table name if necessary
 $post_changes_query = $wpdb->prepare(
    "SELECT post_id, action FROM $post_changes_table WHERE needs_transfer = %d",
    1
);
 $post_changes = $wpdb->get_results($post_changes_query, ARRAY_A);

     // Separate post IDs based on action (added or deleted)
// Initialize arrays to store post IDs for different actions

// Iterate through post changes and categorize by action
foreach ($post_changes as $change) {
if ($change['action'] === 'added') {
    $sql_queries['added_post_ids'][] = $change['post_id'];
} elseif ($change['action'] === 'deleted') {
    $sql_queries['deleted_post_ids'][] = $change['post_id'];
} 
}
