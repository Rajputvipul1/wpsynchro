...
class DatabaseSync
{
    ...
    public function runDatabaseSync(&$migration, &$job)
    {
        ...
        while ($this->timer->shouldContinueWithLastrunTime($lastrun_time)) {
            ...
            foreach ($this->job->from_dbmasterdata as &$table) {
                // error_log('db master data: ' . print_r($table, true));
    ...
    {
        // error_log('table data : ' .print_r($body->sql_inserts,true));
        ...
        // error_log('database transport response: ' . print_r($database_result, true));
