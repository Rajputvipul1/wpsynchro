...
if ( !class_exists('WPSynchroPuc_v4p9_UpdateChecker', false) ):

	abstract class WPSynchroPuc_v4p9_UpdateChecker {
		...
		/**
		 * @var WPSynchroPuc_v4p9_InstalledPackage
		 */
		protected $package;

		/**
		 * @var WPSynchroPuc_v4p9_Scheduler
		 */
		public $scheduler;

		/**
		 * @var WPSynchroPuc_v4p9_UpgraderStatus
		 */
		protected $upgraderStatus;

		/**
		 * @var WPSynchroPuc_v4p9_StateStore
		 */
		...
		public function __construct($metadataUrl, $directoryName, $slug = null, $checkPeriod = 12, $optionName = '') {
			...
			$this->upgraderStatus = new WPSynchroPuc_v4p9_UpgraderStatus();
			$this->updateState = new WPSynchroPuc_v4p9_StateStore($this->optionName);
		 ...
		 *
		 * @return WPSynchroPuc_v4p9_InstalledPackage
		 */
		 ...
		 * @return WPSynchroPuc_v4p9_Scheduler
		 */
		 ...
		 *
		 * @return WPSynchroPuc_v4p9_Update|null
		 */
		public function checkForUpdates() {
			...
			add_action('WPSynchroPuc_api_error', array($this, 'collectApiErrors'), 10, 4);
			...
			remove_action('WPSynchroPuc_api_error', array($this, 'collectApiErrors'), 10);
		 ...
		 *
		 * @return WPSynchroPuc_v4p9_StateStore
		 */
		 ...
		 *
		 * @return WPSynchroPuc_v4p9_Update An instance of Update, or NULL when no updates are available.
		 */
		 ...
		 *
		 * @param WPSynchroPuc_v4p9_Metadata|null $update
		 */
		protected function fixSupportedWordpressVersion(WPSynchroPuc_v4p9_Metadata $update = null) {
		 ...
		 *
		 * This method adds the "WPSynchroPuc_" prefix and the "-$slug" suffix to the filter name.
		 * For example, "pre_inject_update" becomes "WPSynchroPuc_pre_inject_update-plugin-slug".
		 *
		 * @param string $baseTag
		 * @return string
		 */
		public function getUniqueName($baseTag) {
			$name = 'WPSynchroPuc_' . $baseTag;
			if ( $this->filterSuffix !== '' ) {
		 ...
		 *
		 * Identical to add_filter(), except it automatically adds the "WPSynchroPuc_" prefix
		 * and the "-$slug" suffix to the filter name. For example, "request_info_result"
		 * becomes "WPSynchroPuc_request_info_result-your_plugin_slug".
		 *
		 * @param string $tag
		 ...
		 * @return array [WPSynchroPuc_v4p9_Metadata|null, array|WP_Error] A metadata instance and the value returned by wp_remote_get().
		 */
		protected function requestMetadata($metaClass, $filterRoot, $queryArgs = []) {
			...
			if ( !is_wp_error($status) ){
				...
				do_action('WPSynchroPuc_api_error', $status, $result, $url, $this->slug);
				$this->triggerError(
		...
		protected function validateApiResponse($result) {
			...
			if ( $result['response']['code'] !== 200 ) {
				return new WP_Error(
					'WPSynchroPuc_unexpected_response_code',
					'HTTP response code is ' . $result['response']['code'] . ' (expected: 200)'
				);
			}

			if ( empty($result['body']) ) {
				return new WP_Error('WPSynchroPuc_empty_response', 'The metadata file appears to be empty.');
			}
		...
		protected function createDebugBarExtension() {
			return new WPSynchroPuc_v4p9_DebugBar_Extension($this);
		}

		/**
		 * Display additional configuration details in the Debug Bar panel.
		 *
		 * @param WPSynchroPuc_v4p9_DebugBar_Panel $panel
		 */
